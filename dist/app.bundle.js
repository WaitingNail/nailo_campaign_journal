...
function home(data){
 const campaigns=[...data.campaigns].sort((a,b)=>(b.sessions.at(-1)?.date||b.startDate||'').localeCompare(a.sessions.at(-1)?.date||a.startDate||''));const featured=campaigns[0];const recent=campaigns.slice(1,4);const chars=allCharacters(data).filter(x=>x.character.mine||x.campaign.role==='gm').slice(0,8);
...
function characters(data,key){const list=allCharacters(data);if(!list.length)return `<section class="front-empty"><span>CHARACTER ARCHIVE</span><h1>角色名鑑還是空的</h1><p>角色資料公開後，便會自動出現在這裡。</p></section>`;let index=Math.max(0,list.findIndex(x=>x.key===key));const selected=list[index],{campaign:c,character:p}=selected;
 return `<section class="character-stage" data-character-index="${index}"><div class="character-stage-bg" style="--portrait-bg:${p.color||'#375963'}"></div><div class="character-feature">${portrait(c,p,'feature')}<div class="character-copy"><span class="front-kicker">${e(systemName(c))} · ${c.role==='played'?'玩家角色':'主持團角色'}</span><h1>${e(p.name)}</h1><p class="character-class">${e(charSubtitle(c,p))}</p><dl><div><dt>所屬團務</dt><dd><a href="#/story/${e(c.id)}">${e(c.title)}</a></dd></div><div><dt>玩家</dt><dd>${e(p.player)}</dd></div><div><dt>GM</dt><dd>${e(c.gm)}</dd></div>${p.ancestry?`<div><dt>種族</dt><dd>${e(p.ancestry)}</dd></div>`:''}${p.background?`<div><dt>背景</dt><dd>${e(p.background)}</dd></div>`:''}</dl><p class="character-description">${e(p.notes||'這名角色的故事，還等著被寫下。')}</p>${p.sheetUrl?`<a class="front-text-link" href="${e(p.sheetUrl)}" target="_blank" rel="noopener noreferrer">開啟角色卡 ${icon('link')}</a>`:''}</div></div><div class="character-selector-wrap"><button class="selector-arrow prev" data-action="char-prev" aria-label="上一位角色">${icon('back')}</button><div class="character-selector" id="character-selector" role="listbox" aria-label="選擇角色">${list.map((x,i)=>`<a role="option" aria-selected="${i===index}" href="#/characters/${x.key}" class="selector-card ${i===index?'selected':''}" data-selector-index="${i}" style="--portrait-bg:${x.character.color||'#375963'}">${portrait(x.campaign,x.character,'selector')}<span><strong>${e(x.character.name)}</strong><small>${e(x.campaign.title)}</small></span></a>`).join('')}</div><button class="selector-arrow next" data-action="char-next" aria-label="下一位角色">${icon('arrow')}</button></div><p class="selector-help">使用左右方向鍵或箭頭循環選擇角色 · ${index+1} / ${list.length}</p></section>`;
}
function story(data,c){if(!c)return `<section class="front-empty"><h1>找不到這場冒險</h1><a class="front-button" href="#/journal/all/all">返回團務誌</a></section>`;return `<article class="story-page"><header class="story-head"><a href="#/journal/${c.role}/${c.system}" class="front-text-link">${icon('back')}返回團務誌</a><span>${e(systemName(c))} · ${c.role==='played'?'PLAYER’S JOURNAL':'GAME MASTER’S JOURNAL'}</span><h1>${e(c.title)}</h1><p>${e(c.description||'這場冒險的介紹尚未寫下。')}</p><div><span>${status(c)}</span><span>GM · ${e(c.gm)}</span><span>${fmt(c.startDate)}</span><span>${c.sessions.length} 回紀錄</span></div></header><section class="story-party"><div class="front-section-head"><div><span>THE PARTY</span><h2>同行的角色</h2></div></div><div class="story-character-row">${c.characters.map(p=>`<a href="#/characters/${portraitKey(c,p)}">${portrait(c,p,'story')}<strong>${e(p.name)}</strong><small>${e(charSubtitle(c,p))}</small></a>`).join('')}</div></section><section class="story-entries"><header><span>SESSION ARCHIVE</span><h2>冒險紀錄</h2></header>${c.sessions.length?c.sessions.map((s,i)=>`<article class="story-entry"><div class="story-entry-no">${String(i+1).padStart(2,'0')}</div><div><time>${fmt(s.date)}</time><h3>${e(s.title)}</h3><p>${e(s.summary||'本回摘要尚未寫下。')}</p>${s.url?`<a class="front-text-link" href="${e(s.url)}" target="_blank" rel="noopener noreferrer">閱讀完整團錄 ${icon('link')}</a>`:''}</div></article>`).join(''):`<div class="front-empty"><p>尚未新增回次紀錄。</p></div>`}</section></article>`}
